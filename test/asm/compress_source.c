...
inline void push_char(char * dst, uint32_t * pos, char c) {
    dst[(*pos)++] = c;
}

inline uint32_t itoa_inverse(char * buffer, uint32_t val) {
    uint32_t i = 0;
    while(val > 0) {
        buffer[i] = '0' + val%10;
        val /= 10;
...
inline uint32_t push_int(char * buffer, uint32_t* pos, uint32_t val) {
    char tmp[16];
...
inline void push_char_count(char * dst, uint32_t* pos, char c, uint32_t count) {
    if(count == 0) return;
    push_char(dst, pos, c);
    if(count == 1) return;
...
inline uint32_t compress(const char * src, uint32_t src_len, char * dst) {
    char last_char = 0;
    uint32_t count = 0;
    uint32_t pos = 0;
    char c;
...
inline void push_char_multiple(char * buffer, uint32_t* pos, char c, char * count) {
    while(*count > 0) {
...
inline void push_decompressed(char * dst, uint32_t* pos, char last_char, uint32_t * count, uint32_t* parsing_state) {
    while(*count > 1) {
...
inline void decompress(const char * src, uint32_t src_len, char * dst) {
    uint32_t count = 0;
    uint32_t pos = 0;
    char c;
    char last_char = 0;
...
inline void finish_push_number(char * res, uint32_t * pos, char value, uint32_t* parsing_state) {
...
inline void normalize_key(char* key, uint32_t* key_len) {
    char * res = key;
    uint32_t count = 0;
    uint32_t i, j = 0;
    flag_t parsing_state = NOT_PARSING;
